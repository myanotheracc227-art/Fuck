...
-- Settings Button (BELOW O BUTTON) - ADD HERE AFTER WINDOW CREATION
...
for i, tabName in ipairs(settingsTabNames) do
	...
	settingsScrollFrame.Visible = (i == 1) -- Only first tab visible by default
...
usernameLabel.Font = Enum.Font.GothamBold -- Already bold, keep it
usernameLabel.TextSize = 15  -- Slightly bigger
...
for i, tabName in ipairs(settingsTabNames) do
	...
	tabButton.LayoutOrder = 1000 + i -- High order to appear at bottom
...
-- Settings Button Click Handler (FIXED - WORKS WITHOUT USER TABS)
local canClickSettings = true

settingsButton.MouseButton1Click:Connect(function()
	if not canClickSettings then return end
	...
	if not settingsOpen then
		...
		-- Rotate ONLY the image inside the ImageButton
		TweenService:Create(settingsButton, TweenInfo.new(0.3, Enum.EasingStyle.Back), {
			ImageRotation = -15
		}):Play()
		
		-- Save current scroll position and visible tab (if any exist)
		...
		-- Show first settings tab (Settings tab with profile)
		for name, frame in pairs(settingsTabs) do
		...
		-- Rotate image back to 0
		TweenService:Create(settingsButton, TweenInfo.new(0.3, Enum.EasingStyle.Back), {
			ImageRotation = 0
		}):Play()
		
		-- Hide settings tab buttons
		for _, tabButton in pairs(settingsTabButtons) do
			tabButton.Visible = false
		end
		
		-- Show normal tab buttons
		for _, tabButton in pairs(Window.TabButtons) do
			tabButton.Visible = true
		end
		
		-- Slide settings container up FIRST
		...
		-- Wait for animation, then hide and restore tabs
		task.delay(0.4, function()
			settingsTabsContainer.Visible = false
			
			-- Restore previously visible tab (if any existed)
